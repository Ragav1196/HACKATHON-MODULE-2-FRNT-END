...
/* this one is used to call a function when clicked outside a element */
/* link to download => https://github.com/airbnb/react-outside-click-handler */
import { context } from "../link/Links";
import SearchIcon from "@mui/icons-material/Search";
import { GetUserType } from "../authentication/UserType";

export function Topbar() {
  // TO HIDE AND SHOW THE USERSLIDEMENU
  const [userSlideMenu, setLoginButton] = useState(true);

  // TO SET THE TITLE IN THE TOP BAR:
  const { title } = useContext(context);

  // TO SET THE SAME TITLE WHAT IT HAD BEFORE ENTERING THE REGISTER PAGE:
  const { setTitleReg } = useContext(context);

  useEffect(() => setTitleReg(Title));
  ...
  return (
    <section className="topbar">
      ...
      <article>
        ...
        <p
          onClick={() => {
            setLoginButton(!userSlideMenu);
          }}
        >
          <AccountCircleIcon />
        </p>
        <UserSlideMenu
          userSlideMenu={userSlideMenu}
...
function UserSlideMenu({ userSlideMenu, setLoginButton }) {
  ...
  // ANIMATION STYLE FOR USERMENU
  const styles = {
    transform: userSlideMenu ? "translateY(-250px)" : "translateY(0px)",
  };

  return (
    <OutsideClickHandler onOutsideClick={() => setLoginButton(true)}>
      <section style={styles} className="loginButton">
        <article>
          <img
